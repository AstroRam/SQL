The following are sql course data mostly from Kaggle.




Sub select or nested Select;

Select EMP_ID, F_NAME, L_NAME, SALARY
From employees
Where SALARY < (select AVG(SALARY) from employees);

Sub select in list of columns:
...
SELECT emp_id, fmame, lname, salary FROM employees where salary < (SELECT AVG(salary) FROM employees); SELECT * FROM ( SELECT emp_id, f_name, l_name, dep_id FROM employees) AS emp4all;
...
SELECT CAST(MyVarcahrCOl AS INT) FROM MyTable
